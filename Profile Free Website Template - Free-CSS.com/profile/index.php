...
$stmt = $conn->prepare("SELECT full_name FROM doctors WHERE d_id = ?");
...
<div>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $existingAppointment = $conn->query("SELECT * FROM `app` WHERE `date` = '$date' AND `d_id` = '$d_id'")->fetch_assoc();
        if ($existingAppointment) {
            // Update existing appointment
            $updateColumns = [];
            foreach ($selectedTimeSlots as $timeSlot) {
                $updateColumns[] = "`$timeSlot` = 'yes'";
            }
            $updateQuery = "UPDATE `app` SET " . implode(",", $updateColumns) . " WHERE `date` = '$date' AND `d_id` = '$d_id'";
            ...
            $sqlColumns = "`d_id`, `date`," . implode(",", array_map(function ($timeSlot) {
                return "`$timeSlot`";
            }, $selectedTimeSlots));
            $sqlValues = "'$d_id', '$date'," . implode(",", array_fill(0, count($selectedTimeSlots), "'yes'"));
            $insertQuery = "INSERT INTO `app` ($sqlColumns) VALUES ($sqlValues)";
